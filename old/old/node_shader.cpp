...
NodeShader::NodeShader(QString node_name)
    :QGraphicsItem()
{
...
NodeShader::NodeShader(const NodeShader &node_item)
{
...
void NodeShader::operator=(const NodeShader &node_item)
{
    this->text = node_item.text;
    this->title_color = node_item.title_color;
    this->proxyContent = node_item.proxyContent;
...
void NodeShader::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
...
QRectF NodeShader::boundingRect() const
{
    return QRectF(
        0,
        0,
...
bool NodeShader::contains(const QPointF &point) const
{
...
void NodeShader::setTitle(QString title)
{
